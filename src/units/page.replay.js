...
function addMouseNode(self) {
    ...
    let _node = self.document.createElement('img')
    _node.src = mouseImageData
    _node.setAttribute('id', '__page_inspector_mouse')
    _node.setAttribute('style', 'position:absolute;top: -20px;left: -20px;z-index:9999;')
    self.document.body.appendChild(_node)
}

export function pageRender(page, self) {
    if (!self && !self.document) return
    let {
        dom,
        size
    } = page
    let _root = self.frameElement
    let container = self.frameElement.parentElement
    //size adjust
    let cWidth = container.offsetWidth
    let cHeight = container.offsetHeight
    _root.width = size.width
    _root.height = size.height
    _root.style.margin = 'auto'
    ...
    self.document.append(plainObjectToDom(dom, self, function (obj, node) {
